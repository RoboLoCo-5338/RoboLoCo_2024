...
public class AutoAimSubsystem {
  ...
  private static final double SPEAKER_TAG_HEIGHT =
      Constants.aprilTagUniqueHeights[1] + Constants.plasticTagLength / 2; // meters
  private static final double DISTANCE_TAG_TO_SPEAKER =
      Constants.SPEAKER_HEIGHT_METERS - SPEAKER_TAG_HEIGHT; // meters
  ...
  private static final double DISTANCE_CAMERA_TO_TAG_Y =
      SPEAKER_TAG_HEIGHT - Constants.CAMERA_HEIGHT_METERS;

  public static double getOptimalAngleRadians(
      double distanceToTag, double angleToTag, double angleCurrent) {
    double resultDegree;
    double numerator =
        DISTANCE_CAMERA_TO_TAG_Y
            - (Constants.ARM_LENGTH * Math.sin(angleCurrent))
            + DISTANCE_TAG_TO_SPEAKER;
    double xDistance = distanceToTag * Math.cos(angleToTag);
    double denominator =
        xDistance + Constants.X_OFFSET_CAMERA_TO_PIVOT + (Constants.ARM_LENGTH * angleCurrent);
  ...
  public static double getOptimalAngleDegrees(
      double distanceToTag, double angleToTag, double angleCurrent) {
    double resultDegree;
    double numerator =
        DISTANCE_CAMERA_TO_TAG_Y
    ...
    double denominator =
        xDistance
            + Constants.X_OFFSET_CAMERA_TO_PIVOT
  ...
  public static double getCurrentAngle() {
    double rotations =
        RobotContainer.m_Arm.getArmPosition(); // only gets motor 1 but it doesnt matter
   ...
   * double distance =
   * Vision.distanceFromTarget(Constants.SPEAKER_HEIGHT_METERS);// the height of
   * the april tag, not hte target, which is good
